...
def running_sum(signature, geneset, library, result=None, compact=False, center=True, interactive_plot=False, plot_type="ES"):
    """
    ...
    running_sum, es, esd, peak, le_genes = enrich.get_SE(np.array(np.abs(signature.iloc[:,0])), sig2i,i2sig, gs)
    ...
    if plot_type == "ES":
        ...
        if not result is None:
            if esd > 0:
                if compact:
                    ax1.text(len(running_sum)/30, 0, "NESD="+"{:.3f}".format(result.loc[geneset,"nesd"]), size=25, bbox={'facecolor':'white','alpha':0.8,'edgecolor':'none','pad':1}, ha='left', va='bottom', zorder=100)
                else:
                    ax1.text(len(running_sum)/30, 0, "NESD="+"{:.3f}".format(result.loc[geneset,"nesd"]), size=20, bbox={'facecolor':'white','alpha':0.8,'edgecolor':'none','pad':1}, ha='left', va='bottom', zorder=100)
            else:
                if compact:
                    ax1.text(len(running_sum)/30, 0, "NESD="+"{:.3f}".format(result.loc[geneset,"nesd"]), size=25, bbox={'facecolor':'white','alpha':0.8,'edgecolor':'none','pad':1}, ha='left', va='top', zorder=100)
                else:
                    ax1.text(len(running_sum)/30, 0, "NESD="+"{:.3f}".format(result.loc[geneset,"nesd"]), size=20, bbox={'facecolor':'white','alpha':0.8,'edgecolor':'none','pad':1}, ha='left', va='top', zorder=100)
        else:
            if esd > 0:
                if compact:
                    ax1.text(len(running_sum)/30, 0, "ESD="+"{:.3f}".format(running_sum[nn]), size=25, bbox={'facecolor':'white','alpha':0.8,'edgecolor':'none','pad':1}, ha='left', va='bottom', zorder=100)
                else:
                    ax1.text(len(running_sum)/30, 0, "ESD="+"{:.3f}".format(running_sum[nn]), size=20, bbox={'facecolor':'white','alpha':0.8,'edgecolor':'none','pad':1}, ha='left', va='bottom', zorder=100)
            else:
                if compact:
                    ax1.text(len(running_sum)/30, 0, "ESD="+"{:.3f}".format(running_sum[nn]), size=25, bbox={'facecolor':'white','alpha':0.8,'edgecolor':'none','pad':1}, ha='left', va='top', zorder=100)
                else:
                    ax1.text(len(running_sum)/30, 0, "ESD="+"{:.3f}".format(running_sum[nn]), size=20, bbox={'facecolor':'white','alpha':0.8,'edgecolor':'none','pad':1}, ha='left', va='top', zorder=100)


    ax1.grid(True, which='both')
    ax1.set(xticks=[])
    plt.title(f"{geneset} - {plot_type}", fontsize=18)
    plt.ylabel("Enrichment Score (ES)" if plot_type == "ES" else "Enrichment Score Derivative (ESD)", fontsize=24 if compact else 16)
    if compact:
...
def top_table(signature, library, result, n=10, center=True, interactive_plot=False, plot_type="ES"):
    """
