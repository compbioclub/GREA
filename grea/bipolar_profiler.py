...
def profile_bipolarity(rank_df, libraries, 
    ...
    t_results, perf_results, pred_results = prob_test(
        df, n_perm=n_perm, high=high, low=low, prefix=prefix)

    return df, t_results, pred_results
...
def _random_permutation_test_by_prob(df, prob=50, n_perm=10):
    t_perf = []
    N = df.shape[0]
...
def _predict_by_prob(df, prob):
    preds = df[(df>prob*0.01).all(axis=1)].index.tolist()
    return preds